...
**NOTE when doing this, the defaults are change to the primitive defaults, so polling is not enabled unless you
...
```shell script
mvn dependency-track -DPollingConfig.enabled=false -Dpolling.attempts=100 -Dpolling.pause=2 -Dpolling.timeUnit=MILLIS
``` 
